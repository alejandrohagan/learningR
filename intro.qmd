...
left_join(df_22,
          df_21,
          join_by(age_20==age_20
                  ,sex==sex)) %>% 
  left_join(df_20,
           join_by( age_20==age
            ,sex==sex)) %>% 
  rename(cohort_22=age_20)

df %>% 
  ggplot(aes(y=popestimate2022,x=age,fill=factor(sex,labels = c("male","female"))))+
  geom_col()+
  geom_vline(xintercept = 18,linetype="dotted",size=1)+
  ...
  labs(
    fill="Gender"
    ,title="Distribution of US Pop. 2022"
    )
