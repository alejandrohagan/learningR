...
If you want to add more than one element you will need to use the `c()` function. What is the c function? -- no one knows. Actually, I think the concensus is that it stands for concatenate but who knows for sure
...
Notice what happens when we added a character with a numeric -- it turned evrerything into character.

This is is called type coercion -- it is something your friend and sometimes a source of frustration.
...
c(beta=1:10,charlie=2:21,1,1,alpha=list(10:1),recursive=TRUE)
